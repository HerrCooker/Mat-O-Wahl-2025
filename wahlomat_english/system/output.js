...
*/
	...
	// Link aus definition_english.js einfügen
...
function setLanguage2(lang){
	language = lang;
	document.writeln("<script type='text/javascript' src='i18n/i18n_"+lang+".js'><\/script>");
	languageChange
...
function fnChangeVotingDouble()
{
	...
	strBtnSrc = $("#votingDouble").hasClass("btn-dark");

	
	if (strBtnSrc)
	// wenn vorher unwichtig -> jetzt doppelt werten
	{
		$("#votingDouble").removeClass( "btn-dark" ).css("background-color", "darkorange");
		$("#jumpToQuestionNr"+(activeQuestion+1)+"").css("font-weight","bold");
	}
	// wenn vorher wichtig -> jetzt wieder auf normal setzen
	else
	{
		$("#votingDouble").addClass( "btn-dark" ).css("background-color", "#a1abb5");
